...
if (isServer && !isDedicated) then {
	...
	player addAction ["<t align='left' color='#F7D358'>Spawn Hummingbird", { _veh = createVehicle [ "sfp_hkp9_sog", player modelToWorld [0,10,0], [], 0, "CAN_COLLIDE" ]; _veh setDir (direction player - 90); _veh setVectorUp surfaceNormal position _veh; _veh ;  [0, "A3\Air_F\Heli_Light_01\Data\Skins\heli_light_01_ext_furious_co.paa"];}];
	player addAction ["<t align='left' color='#F7D358'>Spawn Hunter", { _veh = createVehicle [ "sfp_tgb16", player modelToWorld [0,10,0], [], 0, "CAN_COLLIDE" ]; _veh setDir (direction player - 90); _veh setVectorUp surfaceNormal position _veh; _veh ; [0, "#(rgb,8,8,3)color(0,0,0,1)"]; /*_veh ;  [1, "#(rgb,8,8,3)color(0.64,0.64,0.64,1)"];*/}];
